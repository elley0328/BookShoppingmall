...
<script>
	...
	function fn_show_next_goods(){
		...
		if(array_index <_h_goods_id.length-1)
			array_index++;//다음 클릭시 배열의 인덱스를 1증가시킵니다.
		...
		cur_goods_num.innerHTML=array_index+1;//증가된 인덱스에 대한 배열 요소의 상품번호와 이미지 파일 이름을 가져와 표시
	}//빠른 메뉴의 다음 클릭시 <hidden>태그에 저장된 상품 정보를 가져와 이미지를 표시
