...
<script type="text/javascript">
	...
	function keywordSearch(){
	 ...
	 var value=document.frmSearch.searchWord.value;
		$.ajax({
			type : "get",
			async : true, //false인 경우 동기식으로 처리한다.//false인 경우, 동기식으로 처리
	...
	function displayResult(jsonInfo){
		var count = jsonInfo.keyword.length;//JSON 데이터 개수 구하기
		if(count > 0) {
		    var html = '';
		    for(var i in jsonInfo.keyword){//JSON 데이터를 차례대로 <a>태그를 이용해 키워드 목록을 만들기
			   html += "<a href=\"javascript:select('"+jsonInfo.keyword[i]+"')\">"+jsonInfo.keyword[i]+"</a><br/>";
		    }
		    var listView = document.getElementById("suggestList");
		    listView.innerHTML = html;
		    show('suggest');//<a>태그로 만든 키워드 목록을 <div>태그에 차례대로 표시
		}else{
		    hide('suggest');
		} 
	}
